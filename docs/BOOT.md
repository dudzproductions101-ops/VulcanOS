...
## 2.3 Graphics Initialization
    ...
    Detect GRUB Multiboot2 framebuffer info

    Support 24/32-bit RGB linear framebuffer writes

    Display an early software-rendered boot splash
...
### 5.0 Real hardware boot today

The current build still uses GRUB to launch the kernel, but the generated ISO is usable on real hardware. A hybrid USB image can be produced with `make usb` and written to a removable drive, allowing VulcanOS to boot on x86_64 machines with BIOS or legacy UEFI support.
